-- Drop the old function first if it exists, ignoring errors if it doesn't
drop function if exists get_filtered_offers (text, text[]);

-- Recreate the function with the correct jsonb operator and numeric payout type
create
...
-- Recreate the function with the correct jsonb operator, pagination, and numeric payout type
create
