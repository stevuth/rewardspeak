...
export async function POST(request: NextRequest) {
  // 1. Get the authenticated user with the standard API client
  ...
  // 3. Use the Admin Client for storage and database operations
  // Note: We are not passing request here because admin client uses the service role key
  ...
  // 7. Revalidate paths to show the new avatar (this is a client-side signal now)
  // The client will handle revalidation via router.refresh() or state update.
